...
bot.onText(/\/start/, (msg) => {
    bot.sendMessage(msg.chat.id, 'Send me a YouTube video link, and I will download it in 4K!');
});
...
bot.on('message', async (msg) => {
    ...
    if (cache.has(text)) {
        if (!cache.get(text).sent) {
            bot.sendMessage(chatId, `Your video is ready: ${cache.get(text).link}`);
            cache.get(text).sent = true;
        }
        return;
    }
    ...
    try {
        ...
        ffmpeg()
            ...
            .on('end', () => {
                ...
                const downloadLink = `http://yourserver.com/downloads/${encodedTitle}.mp4`;
                bot.sendMessage(chatId, `Your video is ready: ${downloadLink}
Size: ${fileSize} MB`);
                cache.set(text, { link: downloadLink, sent: false });
                bot.sendMessage(chatId, `Your video is ready: ${downloadLink}`);
            })
            .on('error', (err) => bot.sendMessage(chatId, `FFmpeg Error: ${err.message}`));
    } catch (error) {
